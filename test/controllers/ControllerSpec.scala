...
class SubmissionStubControllerSpec extends UnitSpec {
  "The validation results for a valid testReponse and valid headers should be a success" should {
    "return a valid request object" in {
      val requestJs = ControllerHelper.addExtraRequestHeaderChecks(TestData.validHeadersExample, JsSuccess(TestData.validSubmissionResponse))
      requestJs.isSuccess shouldBe true
      val request = requestJs.get
      request shouldEqual TestData.validSubmissionResponse
    }
  }
